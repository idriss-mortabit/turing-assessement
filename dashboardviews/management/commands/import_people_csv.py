...
from dashboardviews.models import Customer, Order, Images, Products
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if connection.vendor == 'mysql':
            ...
            cursor.execute('TRUNCATE TABLE `{0}`'.format(Customer._meta.db_table))
            cursor.execute('TRUNCATE TABLE `{0}`'.format(Order._meta.db_table))
            cursor.execute('TRUNCATE TABLE `{0}`'.format(Products._meta.db_table))
            cursor.execute('TRUNCATE TABLE `{0}`'.format(Images._meta.db_table))
